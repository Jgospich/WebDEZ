...
if (playercollide)
  // *************** INSTANTIATE CLASSES ********************
  let player = new Player(20, 25, WIDTH / 2, HEIGHT / 2, 'Green', 0, 0);

// adds two different sets of mobs to  mobs array
for (i = 0; i < 20; i++) {
  mobs1.push(new Mob(60, 60, 200, 100, 'Gold', Math.random() * -2, Math.random() * -2));
}
while (mobs2.length < 10) {
  mobs2.push(new Mob(10, 10, 250, 200, 'Silver', Math.random() * -2, Math.random() * -2));
}
...
function draw() {
  ...
  drawText('red', "28px Helvetica", "left", "top", "Timer: " + countUp(30), 600, 0);
